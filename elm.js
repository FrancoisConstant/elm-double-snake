...
var $author$project$Main$KeyPushed = function (a) {
	return {$: 'KeyPushed', a: a};
};
	...
	return $elm$core$Platform$Sub$batch(
		_List_fromArray(
			[
				...
				A2($elm$json$Json$Decode$map, $author$project$Main$KeyPushed, $author$project$Main$keyDecoder)),
...
var $author$project$Main$getNewHeadPosition = F2(
	function (positions, direction) {
		var maybeSnakeHead = $elm$core$List$head(positions);
		if (maybeSnakeHead.$ === 'Nothing') {
			return A2($author$project$Positions$Position, 0, 0);
		} else {
...
var $author$project$Main$isSnakeCrashing = F2(
	function (snake, otherSnake) {
		var snakeBodyPositions = A2($elm$core$List$drop, 1, snake.positions);
		var headPosition = A2(
			$elm$core$Maybe$withDefault,
			{x: -1, y: -1},
		...
		return ((_Utils_cmp(headPosition.x, $author$project$Settings$sizeX) > 0) || ((headPosition.x < 0) || ((_Utils_cmp(headPosition.y, $author$project$Settings$sizeY) > 0) || (headPosition.y < 0)))) || (A2($elm$core$List$member, headPosition, snakeBodyPositions) || A2(
			$elm$core$List$any,
			function (position) {
				return A2($elm$core$List$member, position, otherSnake.positions);
			},
			snake.positions));
	});
...
var $author$project$Main$setSnake = F2(
...
var $author$project$Main$updateTimes = F3(
	function (deltaTime, newFrame, model) {
		var newTotalTime = model.totalTime + deltaTime;
		var newElapsedTimeSinceLastUpdate = newFrame ? 0 : (model.elapsedTimeSinceLastUpdate + deltaTime);
		return _Utils_update(
			model,
			{elapsedTimeSinceLastUpdate: newElapsedTimeSinceLastUpdate, totalTime: newTotalTime});
	});
var $author$project$Main$doUpdateFrame = F2(
	function (timeDelta, model) {
		var snake = model.snake;
		var snake2 = _Utils_update(
			snake,
			{
				directions: $author$project$Main$removePreviousDirection(snake.directions)
			});
		var otherSnakeHead = A2(
			$author$project$Main$getNewHeadPosition,
			model.otherSnake.positions,
			$author$project$Main$getDirection(model.otherSnake));
		var newOtherSnakeDirection = A3(
			...
			return _Utils_Tuple2(
				A3(
					...
					A2(
						...
						score,
						A2(
							$author$project$Main$setOtherSnake,
							otherSnake3,
							A2(
...
var $author$project$Main$updateTimesOnly = F2(
	function (timeDelta, model) {
		return _Utils_Tuple2(
			A3($author$project$Main$updateTimes, timeDelta, false, model),
			$elm$core$Platform$Cmd$none);
	});
...
var $author$project$Main$viewCell = F2(
	function (position, model) {
		var under = _Utils_update(
			position,
			{y: position.y - 1});
		var color = A2($author$project$Main$isSnakeHead, position, model.snake) ? 'snake-head' : (A2($author$project$Main$isSnakeOn, position, model.snake) ? 'snake-body' : (A2($author$project$Main$isAppleOn, position, model) ? 'apple' : (A2($author$project$Main$isSnakeHead, position, model.otherSnake) ? 'snake-2-head' : (A2($author$project$Main$isSnakeOn, position, model.otherSnake) ? 'snake-2-body' : ((A2($author$project$Main$isSnakeOn, under, model.snake) || (A2($author$project$Main$isAppleOn, under, model) || A2($author$project$Main$isSnakeOn, under, model.otherSnake))) ? 'shadow' : '')))));
		return A2(
			$elm$html$Html$td,
			_List_fromArray(
				[
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Use arrows to move.')
							])),
						A2(
						$elm$html$Html$p,
						_List_Nil,
						_List_fromArray(
							[
								$elm$html$Html$text('Avoid the walls. Avoid the black snake.')
							])),
						A2(
						$elm$html$Html$p,
						_List_Nil,
						_List_fromArray(
							[
								$elm$html$Html$text('Eat the apples.')
							]))
					]))
			]));
};
